...
//
	switch (pid = fork()){
		...
		perror("Fork failed, Parent cannot create the child, goodbye:");
		exit(4);
//                   
...
", pid, ppid, ruid, rgid, euid, egid, priority);
	  ...
	  write(msg_pipe[1], msg_buf, strlen(msg_buf)); // parent continues after switch break
